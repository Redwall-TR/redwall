...
export async function generateMetadata({
  ...
  const baslik =
    (data ? (pick(data.baslik, loc) ?? data.baslik.tr) : loc === 'tr' ? 'Güvenlik ve Veri Koruma' : 'Security & Data Protection') +
    ' | Redwall';
  const aciklama =
    loc === 'tr'
      ? 'Redwall altyapısında veri güvenliği, KVKK uyumu ve erişim kontrolü.'
      : 'Data security, KVKK/GDPR alignment and access control in Redwall infrastructure.';
...
export default async function GuvenlikPage({
  ...
  return (
    <>
      <PageHero
        eyebrow={isTr ? 'Güvenlik' : 'Security'}
        title={baslik ?? ''}
        accent="#e63950"
