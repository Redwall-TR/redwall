...
- **Erişim/TLS:** subdomain + Cloudflare (turuncu/proxied), monitör sunucusunda Traefik + Let's Encrypt. `monitor.redwall.tr` kaydı hazır (proxied).
...
|---|---|---|---|
| Traefik | Ters proxi, otomatik HTTPS (LE HTTP-01), tüm UI'ları subdomain'lere yönlendirir | Docker socket (read-only), 80/443 | 80/443 |
...
- Cloudflare "Full" ayarına dokunulmaz (redwall zone geneli); Traefik LE origin sertifikası sunar → uyumlu.
